...
@section('title', 'الطلبات التي تمت المقابلة')
    ...
    <div class="dashboard-body">
        ...
        <div class="main-content">
            <div class="content-header">
                <h1>الطلبات التي تمت المقابلة</h1>
                <p>إدارة الطلبات التي تم إجراء مقابلة مع المتقدمين</p>
            </div>
            ...
            <div class="stats-overview">
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        <h3>{{ \App\Models\Application::whereHas('job', function($query) { $query->where('company_id', auth()->id()); })->where('status', 'interviewed')->count() }}</h3>
                        <p>إجمالي الطلبات</p>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        <h3>{{ \App\Models\Application::whereHas('job', function($query) { $query->where('company_id', auth()->id()); })->where('status', 'interviewed')->where('created_at', '>=', now()->subDays(7))->count() }}</h3>
                        <p>هذا الأسبوع</p>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-content">
                        <h3>{{ \App\Models\Application::whereHas('job', function($query) { $query->where('company_id', auth()->id()); })->where('status', 'interviewed')->where('created_at', '>=', now()->subDays(30))->count() }}</h3>
                        <p>هذا الشهر</p>
                    </div>
                </div>
            </div>
            ...
            <div class="applications-section">
                <div class="section-header">
                    <h2>الطلبات التي تمت المقابلة</h2>
                    <div class="header-actions">
                        <button class="btn btn-primary" onclick="exportInterviewed()">
                ...
                @php
                    $interviewedApplications = \App\Models\Application::whereHas('job', function($query) { 
                        $query->where('company_id', auth()->id()); 
                    })->where('status', 'interviewed')->with(['job', 'applicant'])->latest()->paginate(20);
                @endphp

                @if($interviewedApplications->count() > 0)
                        ...
                        @foreach($interviewedApplications as $application)
                            <div class="application-card">
                                ...
                                <div class="application-actions">
                                    ...
                                    <button class="btn btn-sm btn-warning" onclick="updateStatus({{ $application->id }}, 'shortlisted')">
                ...
                @else
                    <div class="empty-state">
                        ...
                        <h3>لا توجد طلبات تمت المقابلة</h3>
                        <p>لم يتم إجراء مقابلات مع أي متقدمين بعد</p>
                    </div>
                @endif
            </div>
        </div>
