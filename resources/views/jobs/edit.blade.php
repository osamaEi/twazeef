...
<div class="edit-job-container">
    <div class="content-header">
        <h1>تعديل الوظيفة</h1>
        <p>قم بتعديل تفاصيل الوظيفة "{{ $job->title }}"</p>
    </div>

    <div class="form-container">
        <form action="{{ route('jobs.update', $job) }}" method="POST" enctype="multipart/form-data" class="job-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $job->title) }}" required class="form-control">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="location" name="location" value="{{ old('location', $job->location) }}" required class="form-control" placeholder="مثال: الرياض، المملكة العربية السعودية">
                        @error('location')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="type" name="type" required class="form-control">
                            ...
                            <option value="full-time" {{ old('type', $job->type) == 'full-time' ? 'selected' : '' }}>دوام كامل</option>
                            <option value="part-time" {{ old('type', $job->type) == 'part-time' ? 'selected' : '' }}>دوام جزئي</option>
                            <option value="contract" {{ old('type', $job->type) == 'contract' ? 'selected' : '' }}>عقد مؤقت</option>
                            <option value="freelance" {{ old('type', $job->type) == 'freelance' ? 'selected' : '' }}>عمل حر</option>
                        </select>
                        @error('type')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select id="experience_level" name="experience_level" required class="form-control">
                            ...
                            <option value="entry" {{ old('experience_level', $job->experience_level) == 'entry' ? 'selected' : '' }}>مبتدئ (0-2 سنة)</option>
                            <option value="mid" {{ old('experience_level', $job->experience_level) == 'mid' ? 'selected' : '' }}>متوسط (3-5 سنة)</option>
                            <option value="senior" {{ old('experience_level', $job->experience_level) == 'senior' ? 'selected' : '' }}>متقدم (6-10 سنة)</option>
                            <option value="executive" {{ old('experience_level', $job->experience_level) == 'executive' ? 'selected' : '' }}>خبير (أكثر من 10 سنوات)</option>
                        </select>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="salary_min" name="salary_min" value="{{ old('salary_min', $job->salary_min) }}" class="form-control" placeholder="0" min="0" step="0.01">
                        @error('salary_min')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="salary_max" name="salary_max" value="{{ old('salary_max', $job->salary_max) }}" class="form-control" placeholder="0" min="0" step="0.01">
                        @error('salary_max')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select id="salary_currency" name="salary_currency" required class="form-control">
                        ...
                        <option value="SAR" {{ old('salary_currency', $job->salary_currency) == 'SAR' ? 'selected' : '' }}>ريال سعودي (SAR)</option>
                        <option value="USD" {{ old('salary_currency', $job->salary_currency) == 'USD' ? 'selected' : '' }}>دولار أمريكي (USD)</option>
                        <option value="EUR" {{ old('salary_currency', $job->salary_currency) == 'EUR' ? 'selected' : '' }}>يورو (EUR)</option>
                        <option value="AED" {{ old('salary_currency', $job->salary_currency) == 'AED' ? 'selected' : '' }}>درهم إماراتي (AED)</option>
                        <option value="KWD" {{ old('salary_currency', $job->salary_currency) == 'KWD' ? 'selected' : '' }}>دينار كويتي (KWD)</option>
                    </select>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <div class="image-upload-container">
                        <div class="image-preview" id="imagePreview">
                            @if($job->hasImage())
                                <img src="{{ $job->image_url }}" alt="صورة الوظيفة الحالية" class="preview-image">
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" rows="8" required class="form-control" placeholder="اكتب وصفاً مفصلاً للوظيفة والمتطلبات...">{{ old('description', $job->description) }}</textarea>
                    @error('description')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="status" name="status" class="form-control">
                            <option value="active" {{ old('status', $job->status) == 'active' ? 'selected' : '' }}>نشطة</option>
                            <option value="paused" {{ old('status', $job->status) == 'paused' ? 'selected' : '' }}>معلقة</option>
                            <option value="draft" {{ old('status', $job->status) == 'draft' ? 'selected' : '' }}>مسودة</option>
                            <option value="closed" {{ old('status', $job->status) == 'closed' ? 'selected' : '' }}>مغلقة</option>
                        </select>
                        @error('status')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="expires_at" name="expires_at" value="{{ old('expires_at', $job->expires_at ? $job->expires_at->format('Y-m-d') : '') }}" class="form-control">
...
// Initialize skills and benefits from existing job data
